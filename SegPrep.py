...
class SegPrep:
    ...
    def changeColorSpace(self, imageList=None, colorReagons=8):
        ...
        for i in range(len(imageList)):
            ...
            for j in range(colorReagons + 1):
                ...
                imageList[i] = np.where(((imageList[i] > currVal) & (imageList[i] <= currVal + regonsOffset)), currVal,
                                        imageList[i])
        self.qountImages = imageList
        self.prevResults = self.qountImages.copy()
        return self.prevResults.copy()

    def morphEadgeDetection(self, imageList=None, openingKernal=None, closingKernal=None, dilatKernal=None,
                            erodeKernal=None):
    ...
    def findBorders(self, imageList=None, connectivity=1, mode='thick', background=0):
        ...
        for i in range(len(self.borderImgaes)):
            self.borderImgaes[i] = sg.find_boundaries(self.borderImgaes[i], connectivity=connectivity, mode=mode,
                                                      background=background).astype(np.uint8) * 255
        self.prevResults = self.borderImgaes.copy()
        return self.prevResults.copy()

    def floodFill(self, imageList=None, seed_point=(0, 0), new_value=255, selem=None, connectivity=None, tolerance=None,
                  in_place=False, inplace=None):
        if (imageList == None):
            imageList = self.prevResults.copy()
    ...
    def clearBoarders(self, imageList=None, buffer_size=0, bgval=0, in_place=False, mask=None):
        ...
        for i in range(len(self.clearBorderImgaes)):
            self.clearBorderImgaes[i] = sg.clear_border(self.clearBorderImgaes[i], buffer_size=buffer_size, bgval=bgval,
                                                        in_place=in_place, mask=mask)
        ...
        for i in range(len(regionTable)):
            ...
            for _, region in enumerate(regionTable[i]):
                ...
                if region.area >= minArea:
                    ...
                    if ((maxc - minc) / (maxr - minr)) > 5 or (maxr - minr) / (
                            maxc - minc) > 4.5:  # aspect ratio distorted
                        continue
    ...
    def drawRec(self, sortedBoxTable, imageList=None):
        ...
        for i, table in enumerate(sortedBoxTable):
            ...
            for j in range(len(table)):
                ...
                ax.annotate(j + 1, (column_center, row_center), color='w', weight='bold', fontsize=6, ha='center',
                            va='center')
